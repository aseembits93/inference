...
BASE_MODEL_ID = os.environ.get("PARITY_BASE_MODEL_ID", MODEL_ID)
CANDIDATE_MODEL_ID = os.environ.get("PARITY_CANDIDATE_MODEL_ID", MODEL_ID)
CONFIDENCE = 0.4
BOX_DRIFT_PX = int(os.environ.get("PARITY_BOX_DRIFT_PX", "5"))
PY = sys.executable
...
def do_compare(base_path: str, candidate_path: str) -> None:
    ...
    relaxed_matched = relaxed_full_match_images = 0
    ...
    for base_record, candidate_record in zip(base_iter, candidate_iter):
        ...
        relaxed_matched += len(relaxed_matches)
        relaxed_box_drifts.extend(drift for _, _, drift in relaxed_matches)
        if n_base == n_candidate == len(relaxed_matches):
            relaxed_full_match_images += 1
