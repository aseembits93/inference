...
_TOPK_PAD = 128
_CLASS_BLOCK = 128
_TOPK_QUERY_BLOCK = int(os.environ.get("RFDETR_TRITON_POSTPROC_TOPK_QUERY_BLOCK", "14"))
_TOPK_PARTIAL_NUM_WARPS = int(
    os.environ.get("RFDETR_TRITON_POSTPROC_TOPK_PARTIAL_NUM_WARPS", "4")
)
_TOPK_PARTIAL_NUM_STAGES = int(
    os.environ.get("RFDETR_TRITON_POSTPROC_TOPK_PARTIAL_NUM_STAGES", "1")
)
_TOPK_MERGE_NUM_WARPS = int(
    os.environ.get("RFDETR_TRITON_POSTPROC_TOPK_MERGE_NUM_WARPS", "4")
)
_TOPK_MERGE_NUM_STAGES = int(
    os.environ.get("RFDETR_TRITON_POSTPROC_TOPK_MERGE_NUM_STAGES", "1")
)
_MASK_TILE_H = int(os.environ.get("RFDETR_TRITON_POSTPROC_MASK_TILE_H", "8"))
_MASK_TILE_W = int(os.environ.get("RFDETR_TRITON_POSTPROC_MASK_TILE_W", "64"))
_MASK_POSTPROC_NUM_STAGES = int(
    os.environ.get("RFDETR_TRITON_POSTPROC_MASK_NUM_STAGES", "1")
)
_RLE_TILE_H = 32
_RLE_TILE_W = 16
_RLE_MERGE_TILE = 32
_MAX_U32 = 0xFFFFFFFF
# The combined TRT+dense-postproc graph path rotates across fast-preprocess
# slots and keeps graph-owned scratch/output buffers live until the deferred
# adapter consumer drains them. On the current depth-2 workflow path, a depth
# of 4 is the first value that consistently avoids reuse pressure; 5 adds
# overhead without further benefit. Keep depth-1 at the prior default of 3.
_PIPELINE_DEPTH = int(os.environ.get("RFDETR_PIPELINE_DEPTH", "1"))
_DEFAULT_POSTPROC_GRAPH_RING_DEPTH = max(3, 2 * _PIPELINE_DEPTH)
_POSTPROC_GRAPH_RING_DEPTH = max(
    1,
    int(
        os.environ.get(
            "RFDETR_TRITON_POSTPROC_GRAPH_RING_DEPTH",
            str(_DEFAULT_POSTPROC_GRAPH_RING_DEPTH),
        )
    ),
)
...
@dataclass
class RFDETRTritonPostprocGraphState:
...
@dataclass
class RFDETRTritonPostprocGraphRing:
    states: List[Optional[RFDETRTritonPostprocGraphState]]
    next_idx: int = 0
...
if TRITON_AVAILABLE:
    ...
    @triton.jit
    def rfdetr_topk_partial_triton_kernel(
        logits_ptr,
        partial_topk_ptr,
        logits_stride_q,
        ...
        # Exact top-k over a query slice. Global exactness is preserved by
        # merging these per-slice top-k sets in a second kernel.
        pid = tl.program_id(0)
        query_start = pid * QUERY_BLOCK
        top_packed = tl.zeros((TOPK_PAD,), dtype=tl.int64)
        class_offsets = tl.arange(0, CLASS_BLOCK)

        for q_offset in tl.range(0, QUERY_BLOCK, num_stages=1):
            q = query_start + q_offset
            valid_query = q < NUM_QUERIES
            logit = tl.load(
                logits_ptr + q * logits_stride_q + class_offsets,
                mask=valid_query & (class_offsets < NUM_CLASSES_TOTAL),
                other=-float("inf"),
            )
            abs_l = tl.abs(logit)
            z = tl.exp(-abs_l)
            ...
            packed = tl.where(
                valid_query & (class_offsets < NUM_CLASSES_TOTAL),
                (conf_bits.to(tl.int64) << 32) | flat_idx.to(tl.int64),
                tl.zeros((CLASS_BLOCK,), dtype=tl.int64),
            )
            merged = tl.reshape(
                tl.join(top_packed, packed),
                (TOPK_PAD + CLASS_BLOCK,),
            )
            top_packed = tl.topk(merged, k=TOPK_PAD)

        rank_offsets = tl.arange(0, TOPK_PAD)
        tl.store(partial_topk_ptr + pid * TOPK_PAD + rank_offsets, top_packed)

    @triton.jit
    def rfdetr_topk_merge_finalize_triton_kernel(
        partial_topk_ptr,
        bboxes_ptr,
        threshold_ptr,
        class_map_ptr,
        ...
        # Exact reference ranking over the flattened (Q, C) sigmoid grid.
        top_packed = tl.zeros((TOPK_PAD,), dtype=tl.int64)
        rank_offsets = tl.arange(0, TOPK_PAD)
        num_classes_total = tl.full((), NUM_CLASSES_TOTAL, tl.int32)
        valid_class_limit_i32 = valid_class_limit.to(tl.int32)

        for block_idx in tl.range(0, NUM_QUERY_BLOCKS, num_stages=1):
            packed = tl.load(partial_topk_ptr + block_idx * TOPK_PAD + rank_offsets)
            merged = tl.reshape(tl.join(top_packed, packed), (TOPK_PAD + TOPK_PAD,))
            top_packed = tl.topk(merged, k=TOPK_PAD)

        denorm_w_f = denorm_w.to(tl.float32)
        denorm_h_f = denorm_h.to(tl.float32)
        valid_rank = rank_offsets < NUM_QUERIES
        packed = top_packed
        ...
        if HAS_REMAPPING:
            mapped_class = tl.load(class_map_ptr + raw_class, mask=valid_rank, other=-1)
            valid = valid_rank & (mapped_class >= 0)
        else:
            mapped_class = raw_class
            valid = valid_rank & (raw_class < valid_class_limit_i32)

        if PER_CLASS:
            safe_class = tl.where(valid, mapped_class, 0)
            threshold = tl.load(threshold_ptr + safe_class, mask=valid_rank, other=0.0)
        else:
            threshold = tl.load(threshold_ptr)

        keep = valid & (conf > threshold)
        keep_i32 = keep.to(tl.int32)
        keep_prefix = tl.cumsum(keep_i32, axis=0)
        keep_index = keep_prefix - 1
        keep_count = tl.sum(keep_i32, axis=0)

        bbox_base = bboxes_ptr + query_idx * bboxes_stride_q
        cx_pct = tl.load(bbox_base + 0, mask=keep, other=0.0)
        cy_pct = tl.load(bbox_base + 1, mask=keep, other=0.0)
        w_pct = tl.load(bbox_base + 2, mask=keep, other=0.0)
        h_pct = tl.load(bbox_base + 3, mask=keep, other=0.0)
        ...
        x1_i = tl.where(
            ((x1_r - x1) == 0.5) & ((x1_i & 1) != 0), x1_i - 1, x1_i
        )
        y1_i = tl.where(
            ((y1_r - y1) == 0.5) & ((y1_i & 1) != 0), y1_i - 1, y1_i
        )
        x2_i = tl.where(
            ((x2_r - x2) == 0.5) & ((x2_i & 1) != 0), x2_i - 1, x2_i
        )
        y2_i = tl.where(
            ((y2_r - y2) == 0.5) & ((y2_i & 1) != 0), y2_i - 1, y2_i
        )

        base = keep_index * 6
        tl.store(selected_q_ptr + keep_index, query_idx, mask=keep)
        tl.store(combined_out_ptr + base + 0, x1_i, mask=keep)
        tl.store(combined_out_ptr + base + 1, y1_i, mask=keep)
        tl.store(combined_out_ptr + base + 2, x2_i, mask=keep)
        tl.store(combined_out_ptr + base + 3, y2_i, mask=keep)
        tl.store(combined_out_ptr + base + 4, conf_bits, mask=keep)
        tl.store(combined_out_ptr + base + 5, mapped_class, mask=keep)
        tl.store(counter_ptr, keep_count)

        inactive = valid_rank & (rank_offsets >= keep_count)
        tail_base = rank_offsets * 6
        tl.store(selected_q_ptr + rank_offsets, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 0, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 1, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 2, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 3, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 4, 0, mask=inactive)
        tl.store(combined_out_ptr + tail_base + 5, -1, mask=inactive)

    @triton.jit
    def rfdetr_mask_postproc_triton_kernel(
        selected_q_ptr,
        masks_ptr,
        y_indices_ptr,
        y_weights_ptr,
        ...
        pid_tile_y = tl.program_id(1)
        pid_tile_x = tl.program_id(2)
        keep_count = tl.load(counter_ptr)
        if pid_det >= keep_count:
            return

        out_y = pid_tile_y * MASK_TILE_H
        out_x = pid_tile_x * MASK_TILE_W
        ...
        y_mask = y < orig_h
        x_mask = x < orig_w
        selected_q = tl.load(selected_q_ptr + pid_det)
        ...
        if PACK_DENSE_MASKS:
            ...
            out_ptr = (
                ...
                + (out_x // 8 + packed_col_offsets)[None, :] * mask_out_stride_w
            )
            tl.store(
                out_ptr,
                packed,
...
_POSTPROC_GRAPH_RING_CACHE: dict = {}
...
def _get_scratch_buffers(
    ...
    shape = (num_queries, orig_h, mask_w, selection_block, num_query_blocks)
    ...
    _, combined, mask_bin, counter, selected_queries, partial_topk = cached
    return combined, mask_bin, counter, selected_queries, partial_topk


def _allocate_scratch_buffers(
    num_queries: int,
    orig_h: int,
    orig_w: int,
    device: torch.device,
    ...
    mask_bin = torch.empty((num_queries, orig_h, mask_w), dtype=torch.uint8, device=device)
    counter = torch.empty((1,), dtype=torch.int32, device=device)
    selected_queries = torch.empty((num_queries,), dtype=torch.int32, device=device)
    partial_topk = torch.empty(
        (num_query_blocks, selection_block), dtype=torch.int64, device=device
    )
    return combined, mask_bin, counter, selected_queries, partial_topk
...
def _dense_postproc_cuda_graph_eligible(
    *,
    bboxes: torch.Tensor,
    emit_rle: bool,
    pack_dense_masks: bool,
) -> bool:
    if emit_rle or not pack_dense_masks:
        return False
    return getattr(bboxes, "_trt_graph_state", None) is not None


def _dense_postproc_graph_cache_key(
    *,
    ...
    return (
        ...
        int(logits_2d.data_ptr()),
        int(bboxes_2d.data_ptr()),
        int(masks_3d.data_ptr()),
        int(thr_tensor.data_ptr()),
        int(cmap.data_ptr()),
        int(y_indices.data_ptr()),
        int(y_weights.data_ptr()),
        int(y_counts.data_ptr()),
        int(x_indices.data_ptr()),
        int(x_weights.data_ptr()),
        int(x_counts.data_ptr()),
        geometry,
        per_class,
        has_remap,
        simple_mask_fastpath,
        num_queries,
        num_classes_total,
        mask_h,
        mask_w,
...
def _launch_dense_split_postproc(
    *,
    logits_2d: torch.Tensor,
    bboxes_2d: torch.Tensor,
    masks_3d: torch.Tensor,
    ...
    max_y_taps: int,
    max_x_taps: int,
    selection_done_event: Optional["torch.cuda.Event"] = None,
    ...
    rfdetr_topk_merge_finalize_triton_kernel[(1,)](
        ...
        NUM_CLASSES_TOTAL=num_classes_total,
        TOPK_PAD=selection_block,
        num_warps=_TOPK_MERGE_NUM_WARPS,
        num_stages=_TOPK_MERGE_NUM_STAGES,
    )
    current_stream = torch.cuda.current_stream(device)
    if selection_done_event is not None:
        selection_done_event.record(current_stream)
    grid = (
        num_queries,
        triton.cdiv(orig_h, _MASK_TILE_H),
        triton.cdiv(orig_w, _MASK_TILE_W),
    )
    rfdetr_mask_postproc_triton_kernel[grid](
        selected_queries,
        masks_3d,
        y_indices,
        y_weights,
        ...
        mask_bin.stride(1),
        mask_bin.stride(2),
        PACK_DENSE_MASKS=1 if pack_dense_masks else 0,
        ...
        num_warps=4,
        num_stages=_MASK_POSTPROC_NUM_STAGES,
    )
    if done_event is not None:
        done_event.record(current_stream)


def _capture_dense_postproc_graph_state(
    *,
    logits_2d: torch.Tensor,
    bboxes_2d: torch.Tensor,
    masks_3d: torch.Tensor,
    ...
    max_y_taps: int,
    max_x_taps: int,
) -> RFDETRTritonPostprocGraphState:
    combined, mask_bin, counter, selected_queries, partial_topk = (
        _allocate_scratch_buffers(
            num_queries=num_queries,
            orig_h=orig_h,
    ...
    cuda_graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(cuda_graph, stream=current_stream):
        _launch_dense_split_postproc(
            logits_2d=logits_2d,
            bboxes_2d=bboxes_2d,
            masks_3d=masks_3d,
    ...
    return RFDETRTritonPostprocGraphState(
...
def _get_dense_postproc_graph_state(
    *,
    ...
    ring = _POSTPROC_GRAPH_RING_CACHE.get(cache_key)
    if ring is None:
        ring = RFDETRTritonPostprocGraphRing(
            states=[None] * _POSTPROC_GRAPH_RING_DEPTH
        )
        _POSTPROC_GRAPH_RING_CACHE[cache_key] = ring
    state_index = ring.next_idx
    ring.next_idx = (state_index + 1) % len(ring.states)
    state = ring.states[state_index]
    if state is None:
        state = _capture_dense_postproc_graph_state(
            logits_2d=logits_2d,
            bboxes_2d=bboxes_2d,
            masks_3d=masks_3d,
            thr_tensor=thr_tensor,
...
def rfdetr_triton_postproc(
    ...
    top-k output. ``selection_done_event`` fires once count + boxes/classes are
    ready on the split dense path. When ``emit_rle`` is true, ``rle_counts`` and
    ``rle_lengths`` hold COCO-style uncompressed run-length counts for each
    surviving detection.
    """
    ...
    num_query_blocks = triton.cdiv(num_queries, _TOPK_QUERY_BLOCK)
    ...
    orig_w, orig_h = orig_size_wh
    combined, mask_bin, counter, selected_queries, partial_topk = _get_scratch_buffers(
        num_queries=num_queries,
        orig_h=orig_h,
        orig_w=orig_w,
        device=device,
    ...
    if emit_rle:
        ...
        if _dense_postproc_cuda_graph_eligible(
            bboxes=bboxes,
            emit_rle=emit_rle,
            pack_dense_masks=pack_dense_masks,
        ):
            graph_state = _get_dense_postproc_graph_state(
                cache_key=_dense_postproc_graph_cache_key(
                    logits_2d=logits_2d,
                    bboxes_2d=bboxes_2d,
                    masks_3d=masks_3d,
                    thr_tensor=thr_tensor,
            ...
            graph_state.cuda_graph.replay()
            combined = graph_state.combined
            mask_bin = graph_state.mask_bin
            counter = graph_state.counter
            selection_done_event = graph_state.selection_done_event
            done_event = graph_state.done_event
        else:
            selection_done_event = torch.cuda.Event()
            done_event = torch.cuda.Event()
            _launch_dense_split_postproc(
