...
def build_deferred_dense_postproc_detection(
    *,
    combined: torch.Tensor,
    mask_packed_gpu: torch.Tensor,
    ...
    empty_xyxy = torch.empty((0, 4), dtype=torch.int32, device=combined.device)
    empty_conf = torch.empty((0,), dtype=torch.float32, device=combined.device)
    empty_cls = torch.empty((0,), dtype=torch.int32, device=combined.device)
    detections = InstanceDetections(
        ...
        mask=LazyInstancesRLEMasks(
            ...
            mask_packed_gpu=mask_packed_gpu,
            mask_packed_width=orig_w,
            done_event=done_event,
        ),
    )
    detections.__dict__["_combined_gpu"] = combined
    detections.__dict__["_mask_packed_gpu"] = mask_packed_gpu
    detections.__dict__["_counter_gpu"] = counter
    detections.__dict__["_defer_count_to_adapter"] = True
    detections.__dict__["_selection_done_event"] = (
        selection_done_event if selection_done_event is not None else done_event
    )
